...
const InventoryCellDisplay = ({ item, editClick, removeClick, wideView, loading, userHasWritePermissions }) => {
    return (
        <>
            {
                !loading ?
                    <React.Fragment>
                        <TableCell align={userHasWritePermissions ? "center" : "right" } width="30%">{item.count}</TableCell>
